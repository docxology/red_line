...
from red_line.contracts import validate_proposed_red_lines
...
def main(argv: list[str] | None = None) -> int:
    ...
    argparse.ArgumentParser(description="Validate the non-adopted candidate decision ledger.").parse_args(
        argv
    )
    errors = validate_proposed_red_lines(ROOT)
    if errors:
        print("\n".join(f"ERROR: {error}" for error in errors))
        return 1
    print("proposed red-line ledger: valid; all six candidates remain non-adopted")
    return 0
