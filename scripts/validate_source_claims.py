...
from red_line.contracts import validate_source_claims
...
def main(argv: list[str] | None = None) -> int:
    ...
    argparse.ArgumentParser(
        description="Validate the machine-readable source/claim ledger against citations."
    ).parse_args(argv)
    errors = validate_source_claims(ROOT)
    if errors:
        print("\n".join(f"ERROR: {error}" for error in errors))
        return 1
    print("source/claim ledger: valid")
    return 0
